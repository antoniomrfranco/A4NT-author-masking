...
def disp_gen_samples(device, modelGen, modelEval, dp, misc, maxlen=100, n_disp=5, atoms='char', append_tensor=None):
    ...
    for i in xrange(len(lens)):
        ...
        print '  Out : %6s --> %s' % (misc['ix_to_auth'][1-auths[i].item()], jc.join([ix_to_char[c[i].item()] for c in char_outs[:gen_lens[i]] if c[i].item() in ix_to_char]))
        print '  Rev : %6s --> %s\n' % (misc['ix_to_auth'][auths[i].item()], jc.join([ix_to_char[c[i].item()] for c in rev_char_outs[:rev_lens[i]] if c[i].item() in ix_to_char]))
    print '\n-------------------------------------------------------------------------------------------------'
    modelGen.train()
    modelEval.train()
